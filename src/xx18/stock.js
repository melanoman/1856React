...
function WealthRow(props, fs) {
   return <tr>
    ...
    <td colspan='4' />
    <td class="breaker" />
    {props.board.players.map(x=><td style={fs}>{x.wealth}</td>)}
  </tr>
}
...
function SizeRow(props, fs) {
  return <tr>
    <td>{svgCert(props.net.ht(30), "#", 'black', 2, 'black', 'gray')}</td>
    <td class="breaker" />
    <td colspan='4' style={fs}>MAX {props.board.portfolioLimit}</td>
    <td class="breaker" />
    {props.board.players.map(x=><td style={fs}>{x.port}</td>)}
  </tr>
}